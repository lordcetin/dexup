...
export async function GET(req:NextRequest) {
  ...
  const responseData = await fetch('https://pro-api.coingecko.com/api/v3/onchain/networks/trending_pools?include=base_token%2C%20quote_token%2C%20dex%2C%20network',{
    method:'GET',
  ...
  const agdas = newPair
  await client.set(`trendingPoolData`,JSON.stringify(agdas))

  fs.writeFileSync('./config/veri.json', JSON.stringify(agdas));
